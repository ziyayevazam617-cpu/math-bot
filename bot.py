...
async def start_topic_question(user_id, first_name, topic, message=None, callback_message=None):
    ...
    text = (
        ...
        f"üìù Misol: {example_text}\n\n"
        f"To'g'ri javobni tanlang üëá"
    )
...
# ==================== TOPICS ====================
TOPICS = {
    "add_sub": "‚ûï Qo'shish/Ayirish",
    "mul_div": "‚úñÔ∏è Ko'paytirish/Bo'lish",
    "percent": "% Foizlar",
    "fraction": "¬Ω Kasrlar",
    "power": "x¬≤ Darajalar",
    "sqrt": "‚àö Kvadrat ildiz",
    "linear_eq": "üî§ Chiziqli tenglama",
    "quad_eq": "üî§ Kvadrat tenglama",
    "system_eq": "üîó Tenglamalar sistemasi",
    "triangle": "üî∫ Uchburchak",
    "rectangle": "‚ñ≠ To'rtburchak",
    "circle": "‚≠ï Doira",
    "ratio": "‚öñÔ∏è Nisbat",
    "average": "üìä O'rtacha qiymat",
    "negative": "‚ûñ Manfiy sonlar",
    "speed": "üöó Tezlik-vaqt-masofa",
    "bank_percent": "üè¶ Foiz o'sishi",
    "trig": "üìê Trigonometriya",
    "log": "üìà Logarifm",
    "expo_eq": "üì∂ Ko'rsatkichli tenglama",
    "arith_prog": "üî¢ Arifmetik progressiya",
    "geom_prog": "üî¢ Geometrik progressiya",
    "combinatorics": "üé≤ Kombinatorika",
}
...
HINTS = {
    ...
    "percent": "Foizni 100 ga bo'lib, songa ko'paytiring: son √ó foiz √∑ 100.",
    ...
    "triangle": "Uchburchak yuzasi = (asos √ó balandlik) √∑ 2.",
    "rectangle": "To'rtburchak yuzasi = tomon √ó tomon.",
    "circle": "Doira yuzasi = œÄ √ó r √ó r.",
    ...
    "speed": "Masofa = tezlik √ó vaqt.",
    "bank_percent": "Foiz summasi = depozit √ó foiz √∑ 100.",
    ...
    "arith_prog": "a_n = a1 + (n-1) √ó d formulasidan foydalaning.",
    "geom_prog": "a_n = a1 √ó q^(n-1) formulasidan foydalaning.",
...
FORMULAS = {
    "add_sub": (
        ...
        "ï a ? a = 0\n\n"
        "?? XOSSALAR\n"
        "ï a + b = b + a ó o'rin almashtirish\n"
        "ï (a + b) + c = a + (b + c) ó guruhlash\n"
        "ï a ? b = a + (?b)\n"
        "ï a ? b ? b ? a\n\n"
        "?? MANFIY SONLAR\n"
        "ï (+a) + (+b) = a + b\n"
    ...
    "percent": (
        ...
        "?? ASOSIY FORMULALAR\n"
        "ï p% = p / 100\n"
        "ï Sonning p% i = Son ? p / 100\n"
        "ï A son B ning necha foizi: p = A / B ? 100%\n"
        "ï Butun son = qism ? 100 / p\n\n"
        "?? FOIZGA OSHIRISH\n"
        "ï Yangi = Eski ? (1 + p/100)\n\n"
        "?? FOIZGA KAMAYTIRISH\n"
        "ï Yangi = Eski ? (1 ? p/100)\n\n"
        "?? O'ZGARISH FOIZI\n"
        "ï O'zgarish % = (Yangi ? Eski) / Eski ? 100%\n\n"
        "?? KETMA-KET FOIZ O'ZGARISHI\n"
        "ï Yangi = Eski ? (1 ± p?/100) ? (1 ± p?/100)"
    ),
    ...
    "sqrt": (
        ...
        "ï v(ab) = va ? vb\n"
        "ï v(a/b) = va / vb\n"
        "ï ?v(a?) = a ó n toq bo'lsa\n"
        "ï ?v(a?) = |a| ó n juft bo'lsa\n\n"
        "?? SODDALASHTIRISH\n"
        "ï v(a?b) = |a|vb\n"
        "ï v12 = 2v3\n\n"
    ...
    "linear_eq": (
        ...
        "?? XOSSALAR\n"
        "ï Tenglamaning ikkala tomoniga bir xil son qo'shish mumkin.\n"
        "ï Tenglamaning ikkala tomonini bir xil nolmas songa ko'paytirish mumkin."
    ),
    ...
    "quad_eq": (
        ...
        "ï x? = (?b ? vD)/(2a)\n\n"
        "?? HOLATLAR\n"
        "ï D > 0 > 2 ta haqiqiy ildiz\n"
        "ï D = 0 > 1 ta haqiqiy ildiz\n"
        "ï D < 0 > haqiqiy ildiz yo'q\n\n"
        "?? VIETA\n"
        "ï x? + x? = ?b/a\n"
        "ï x?x? = c/a\n\n"
        "?? QISQA KO'PAYTIRISH\n"
    ...
    "system_eq": (
        ...
        "ï a?x+b?y=c?\n\n"
        "?? USULLAR\n"
        "ï O'rniga qo'yish usuli\n"
        "ï Qo'shish/ayirish usuli\n"
        "ï Grafik usul\n\n"
        "?? KRAMER QOIDASI\n"
        "ï D = a?b??a?b?\n"
        "ï Dx = c?b??c?b?\n"
        ...
        "ï D ? 0\n\n"
        "?? YECHIMLAR\n"
        "ï Chiziqlar kesishsa > 1 ta yechim\n"
        "ï Parallel bo'lsa > yechim yo'q\n"
        "ï Ustma-ust tushsa > cheksiz ko'p yechim"
    ),
    ...
    "ratio": (
        ...
        "ï xy=k\n\n"
        "?? MASSHTAB\n"
        "ï Masshtab = xaritadagi masofa / haqiqiy masofa"
    ),
    ...
    "negative": (
        ...
        "ï (?)?(+) = ?\n\n"
        "?? MASOFA\n"
        "ï |a?b| ó a va b orasidagi masofa"
    ),

    "speed": (
        "?? TEZLIK ó VAQT ó MASOFA\n\n"
        "?? ASOSIY\n"
        "ï S=vt\n"
        "ï v=S/t\n"
        "ï t=S/v\n\n"
        "?? O'RTACHA TEZLIK\n"
        "ï v???=umumiy masofa/umumiy vaqt\n\n"
        "?? BIRLIKLAR\n"
        "ï 1 m/s=3.6 km/soat\n"
        "ï 1 km/soat=5/18 m/s"
    ),
    ...
    "bank_percent": (
        ...
        "ï A=P(1+r)^t\n\n"
        "P ó boshlang'ich summa\n"
        "r ó foiz stavkasi o'nli ko'rinishda\n"
        "n ó yiliga kapitalizatsiya soni\n"
        "t ó vaqt\n"
        "A ó yakuniy summa"
    ),
    ...
    "trig": (
        "?? TRIGONOMETRIYA\n\n"
        "?? ASOSIY\n"
        "ï sin?=qarshi katet/gipotenuza\n"
        "ï cos?=yondosh katet/gipotenuza\n"
        "ï tan?=qarshi katet/yondosh katet\n"
        "ï cot?=yondosh katet/qarshi katet\n\n"
        "?? ASOSIY AYNIYAT\n"
        "ï sin??+cos??=1\n"
        "ï tan?=sin?/cos?\n"
        "ï cot?=cos?/sin?\n\n"
        "?? QO'SHISH FORMULALARI\n"
        "ï sin(?+?)=sin?cos?+cos?sin?\n"
        "ï sin(???)=sin?cos??cos?sin?\n"
        "ï cos(?+?)=cos?cos??sin?sin?\n"
        "ï cos(???)=cos?cos?+sin?sin?\n\n"
        "?? IKKI BURCHAK\n"
        "ï sin2?=2sin?cos?\n"
        "ï cos2?=cos???sin??\n"
    ...
    "log": (
        ...
        "ï a^(log?x)=x\n\n"
        "?? KO'PAYTMA\n"
        "ï log?(xy)=log?x+log?y\n\n"
        "?? BO'LISH\n"
        "ï log?(x/y)=log?x?log?y\n\n"
        "?? DARAJA\n"
        "ï log?(x?)=n log?x\n\n"
        "?? ASOSNI ALMASHTIRISH\n"
        "ï log?b=log_cb/log_ca\n\n"
        "?? NATURAL LOGARIFM\n"
        "ï ln x=log?x\n"
        "ï e?2.71828"
    ),
    ...
    "combinatorics": (
        ...
        "ï m?n\n\n"
        "?? QO'SHISH QOIDASI\n"
        "ï m+n ó bir-birini istisno qiluvchi holatlar uchun."
    ),
}

MOTIVATIONS = [
    "‚úÖ To'g'ri! Zo'r ishladingiz!",
    "‚úÖ To'g'ri! Ajoyib!",
    "‚úÖ To'g'ri! Siz iqtidorlisiz!",
    "‚úÖ To'g'ri! Davom eting!",
    "‚úÖ To'g'ri! Zo'r natija!",
    "‚úÖ To'g'ri! Mukammal!",
    "‚úÖ To'g'ri! Shunday davom eting!",
]
...
def ex_mul_div_order_ops(grade):
    ...
    if op_mid == "-":
        ...
        result = mul1 + mul2
    return f"{a} √ó {b} {op_mid} {c} √ó {d} = ? (amallar tartibiga rioya qiling)", result


def ex_mul_div_distributive(grade):
    # Taqsimot qonuni: a √ó (b + c) = a√ób + a√óc
    lo_a, hi_a = _rng(grade, None, (3, 15), (5, 30))
    a = random.randint(lo_a, hi_a)
    b = random.randint(2, 20)
    c = random.randint(2, 20)
    return f"Taqsimot qonunidan foydalanib hisoblang: {a} √ó ({b} + {c}) = ?", a * (b + c)
...
def ex_power_diff_then_square(grade):
    ...
    b = random.randint(1, a - 1)
    return f"({a}‚àí{b})¬≤ = ?", (a - b) ** 2


def ex_power_law_mul(grade):
    # a·µê √ó a‚Åø = a·µê‚Å∫‚Åø - daraja qonuni (8-9 sinf algebra dasturi)
    base = random.randint(2, 5)
    m = random.randint(1, 4)
    n = random.randint(1, 4)
    return f"{base}^{m} √ó {base}^{n} ni {base} ning bitta darajasi ko'rinishida yozsangiz, daraja ko'rsatkichi nechaga teng?", m + n


def ex_power_law_div(grade):
    # a·µê √∑ a‚Åø = a·µê‚Åª‚Åø (m > n bo'lishi shart)
    base = random.randint(2, 5)
    n = random.randint(1, 4)
...
def ex_power_law_value(grade):
    ...
    return f"{base}^{m} √ó {base}^{n} necha songa teng? (avval daraja qonunini qo'llang)", base ** (m + n)
...
def ex_sqrt_product(grade):
    ...
    a, b = random.randint(lo, hi), random.randint(lo, hi)
    return f"‚àö{a*a} √ó ‚àö{b*b} nechaga teng?", a * b
...
def ex_sqrt_estimate(grade):
    ...
    return f"‚àö{n_squared_area} soni qaysi ikkita ketma-ket butun son orasida joylashgan? Kichigini yozing.", low_root


def ex_sqrt_simplify(grade):
    # ‚àö(a¬≤√ób) = a‚àöb ko'rinishida soddalashtirish (b - kvadratsiz son) -
    # 10-11 sinf uchun kuchliroq ildiz bilan ishlash ko'nikmasi
    a = random.randint(2, 10)
    b = random.choice([2, 3, 5, 6, 7, 10, 11, 13, 14, 15])
    n = a * a * b
    return f"‚àö{n} sonini a‚àö{b} ko'rinishida soddalashtiring. a nechaga teng?", a
...
def ex_quad_discriminant(grade):
    ...
    return f"{coef_a} {sign_b} {sign_c} = 0 tenglamaning diskriminanti (D = b¬≤ ‚àí 4ac) nechaga teng?", d
...
def ex_circle_area(grade):
    ...
    return f"Radiusi {r} bo'lgan doira yuzasi (œÄ=22/7 deb oling)?", int(22 * r * r / 7)


def ex_circle_circumference(grade):
    r = _circle_radius(grade)
    return f"Radiusi {r} bo'lgan doiraning aylana uzunligi (œÄ=22/7 deb oling)?", int(2 * 22 * r / 7)
...
def ex_circle_radius_from_area(grade):
    r = _circle_radius(grade)
    area = int(22 * r * r / 7)
    return f"Yuzasi {area} bo'lgan doiraning radiusi nechaga teng? (œÄ=22/7 deb oling)", r


def ex_circle_radius_from_circumference(grade):
    r = _circle_radius(grade)
    circ = int(2 * 22 * r / 7)
    return f"Aylana uzunligi {circ} bo'lgan doiraning radiusi nechaga teng? (œÄ=22/7 deb oling)", r
...
def ex_negative_mul(grade):
    ...
    if random.random() < 0.5:
        b = -b
    return f"({a}) √ó ({b})" if b < 0 else f"({a}) √ó {b}", a * b
...
def ex_negative_mul_chain(grade):
    ...
    for _ in range(n_factors):
        ...
        factors.append(val)
        result *= val
    text = " √ó ".join(f"({f})" if f < 0 else str(f) for f in factors)
...
def ex_trig_identity(grade):
    ...
    return (
        f"sin¬≤Œ± + cos¬≤Œ± = 1 ayniyatiga ko'ra, agar {known}¬≤Œ± = 0 bo'lsa, "
        f"{other}¬≤Œ± nechaga teng?",
        1,
    )
...
def ex_trig_equation(grade):
    ...
    return f"{func}(x¬∞) = {pct}% tenglamani yeching (0¬∞ ‚â§ x ‚â§ 90¬∞, sin(90¬∞)=100% deb hisoblang). x = ?", angle
...
def ex_expo_product_rule(grade):
    ...
    n = x + k
    return f"{base}^x √ó {base}^{k} = {base**n} tenglamani yeching. x = ?", x
...
def ex_geom_infinite_sum(grade):
    # Cheksiz kamayuvchi geometrik progressiya yig'indisi: S = a1 √∑ (1 ‚àí q), |q| < 1.
    # Natija butun son chiqishi uchun q = 1/k va a1 = m√ó(k‚àí1) qilib tanlanadi:
    # S = a1 √∑ (1 ‚àí 1/k) = a1√ók √∑ (k‚àí1) = m√ó(k‚àí1)√ók √∑ (k‚àí1) = m√ók
...
def get_hint_keyboard(topic):
    ...
    builder.button(text="üí° Yordam", callback_data=f"hint_{topic}")
...
async def end_speedtest(user_id, chat_id):
    ...
    if user_id in speedtest_active:
        ...
        await bot.send_message(
            chat_id,
            f"‚è±Ô∏è Vaqt tugadi!\n\n"
            f"60 soniyada siz {count} ta misolni to'g'ri yechdingiz! üéâ\n\n"
...
async def start_handler(message: types.Message):
    ...
    await message.answer(
        f"Salom, {message.from_user.first_name}! ü§ñ\n\n"
...
async def grade_chosen(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text(
        ...
        f"Endi mavzuni tanlang, yoki pastdagi üìã Menu tugmasidan barcha buyruqlarni ko'ring:"
    )
...
async def answer_button_handler(callback: types.CallbackQuery):
    ...
    if chosen == correct_answer:
        ...
        result_text = f"{random.choice(MOTIVATIONS)} üî• Streak: {new_streak} kun"
    else:
        ...
        result_text = f"‚ùå Noto'g'ri. To'g'ri javob: {correct_answer}\nüìñ Yechim: {solution_hint}"
    ...
    await callback.message.edit_text(
        f"{result_text}\n\n"
        f"üìù Keyingi misol{topic_label}: {example_text}\n\n"
        f"To'g'ri javobni tanlang üëá",
...
async def random_handler(message: types.Message):
    ...
    await message.answer(
        f"üé≤ Aralash rejim yoqildi! Har safar boshqa mavzudan savol keladi.\n\n"
        f"üìù Misol ({TOPICS[topic]}): {example_text}\n\n"
        f"To'g'ri javobni tanlang üëá",
        reply_markup=get_answer_keyboard(topic, options)
    )
...
async def challenge_handler(message: types.Message):
    ...
    await message.answer(
        f"‚≠ê Challenge rejimi! Faqat eng qiyin darajadagi savollar.\n\n"
        f"üìù Misol ({TOPICS[topic]}): {example_text}\n\n"
        f"To'g'ri javobni tanlang üëá",
        reply_markup=get_answer_keyboard(topic, options)
    )
...
async def formula_handler(message: types.Message):
    await message.answer("üìò Qaysi mavzu formulasini ko'rmoqchisiz?", reply_markup=get_formula_keyboard())
...
async def formula_chosen(callback: types.CallbackQuery):
    ...
    await callback.message.answer(f"üìò {text}")
    await callback.answer()
...
async def mistakes_handler(message: types.Message):
    ...
    if not rows:
        await message.answer("üìñ Hali xatolaringiz yo'q. Zo'r natija!")
        return

    text = "üìñ Oxirgi xatolaringiz:\n\n"
    for topic, question, correct_answer in rows:
        topic_name = TOPICS.get(topic, topic)
        text += f"‚Ä¢ {topic_name}: {question} ‚Üí to'g'ri javob: {correct_answer}\n"
...
async def topweek_handler(message: types.Message):
    ...
    text = "üìÖ Haftalik reyting:\n\n"
    medals = ["ü•á", "ü•à", "ü•â"]
    for i, (name, week_correct) in enumerate(rows):
        medal = medals[i] if i < 3 else f"{i+1}."
        text += f"{medal} {name} ‚Äî {week_correct} ta to'g'ri\n"
...
async def stats_handler(message: types.Message):
    ...
    await message.answer(
        ...
        f"üéØ Aniqlik: {percent}%\n"
        f"üî• Streak: {streak} kun"
    )
...
async def top_handler(message: types.Message):
    ...
    text = "üèÜ Eng yaxshi 10 ta ishtirokchi:\n\n"
    medals = ["ü•á", "ü•à", "ü•â"]
    for i, (name, correct, wrong) in enumerate(top_users):
        medal = medals[i] if i < 3 else f"{i+1}."
        text += f"{medal} {name} ‚Äî {correct} ta to'g'ri\n"
...
async def joingroup_handler(message: types.Message):
    ...
    await message.answer(f"‚úÖ Siz \"{group[2]}\" guruhiga qo'shildingiz!")
...
async def mystudents_handler(message: types.Message):
    ...
    for code, name in groups:
        ...
        for first_name, correct, wrong in students:
            total = correct + wrong
            pct = round(correct / total * 100) if total > 0 else 0
            text += f"   ‚Ä¢ {first_name} ‚Äî {correct} to'g'ri ({pct}%)\n"
        text += "\n"
...
async def path_handler(message: types.Message):
    ...
    text = "üìö O'quv yo'lingiz:\n\n"
    for t, status in statuses:
        icon = {"done": "‚úÖ", "current": "‚ñ∂Ô∏è", "locked": "üîí"}[status]
    ...
    builder = InlineKeyboardBuilder()
    builder.button(text="‚ñ∂Ô∏è Boshlash", callback_data=f"pathstart_{current_topic}")
...
async def mycode_handler(message: types.Message):
    ...
    await message.answer(
        f"üîó Sizning shaxsiy kodingiz: `{code}`\n\n"
...
async def compare_handler(message: types.Message):
    ...
    if other["user_id"] == message.from_user.id:
        await message.answer("Bu sizning o'z kodingiz üôÇ Do'stingizning kodini kiriting.")
        return
    ...
    await message.answer(
        f"üîó Taqqoslash:\n\n"
        f"üë§ {me['first_name']}: {me['correct']} to'g'ri, {me_pct}% aniqlik, üî•{me['streak']} kun\n"
        f"üë§ {other['first_name']}: {other['correct']} to'g'ri, {other_pct}% aniqlik, üî•{other['streak']} kun"
    )
...
async def speedtest_handler(message: types.Message):
    ...
    await message.answer(
        f"‚è±Ô∏è Tezlik testi boshlandi! 60 soniyada nechta misol yecha olasiz?\n\n"
        f"üìù {example_text} = ?"
    )
...
async def check_answer_handler(message: types.Message):
    ...
    if is_speedtest and user["current_answer"] is not None and message.text and message.text.lstrip("-").isdigit():
        ...
        if user_answer == correct_answer:
            ...
            await message.answer(f"‚ùå Noto'g'ri. To'g'ri javob: {correct_answer}")

        example_text, answer = generate_example(user_id, "add_sub", "medium")
        update_user(user_id, current_answer=answer)
        await message.answer(f"üìù {example_text} = ?")
    else:
        ...
        for user_id, last_played in rows:
            if last_played != today:
                try:
                    await bot.send_message(
                        user_id,
                        "üîî Bugun hali mashq qilmadingiz!\n\n"
                        "5 daqiqa vaqt ajratib, bilimingizni mustahkamlang üí™\n"
...
async def set_bot_commands():
    commands = [
        ...
        types.BotCommand(command="topics", description="üìö Mavzular ro'yxati"),
        types.BotCommand(command="path", description="üó∫Ô∏è O'quv yo'lim"),
        types.BotCommand(command="random", description="üé≤ Aralash rejim"),
        types.BotCommand(command="challenge", description="‚≠ê Qiyin savollar"),
        types.BotCommand(command="speedtest", description="‚è±Ô∏è Tezlik testi"),
        types.BotCommand(command="formula", description="üßÆ Formulalar bazasi"),
        types.BotCommand(command="mistakes", description="üìñ Xatolaringiz"),
        types.BotCommand(command="map", description="üìä Bilim xaritangiz"),
        types.BotCommand(command="stats", description="üìà Statistikangiz"),
        types.BotCommand(command="top", description="üèÜ Umumiy reyting"),
        types.BotCommand(command="topweek", description="üìÖ Haftalik reyting"),
        types.BotCommand(command="mycode", description="üîó Shaxsiy kodim"),
        types.BotCommand(command="compare", description="üîó Do'st bilan solishtirish"),
        types.BotCommand(command="creategroup", description="üè´ Guruh yaratish (o'qituvchi)"),
        types.BotCommand(command="joingroup", description="üè´ Guruhga qo'shilish"),
        types.BotCommand(command="mystudents", description="üè´ O'quvchilarim (o'qituvchi)"),
        types.BotCommand(command="level", description="üéì Sinf darajasi"),
    ]
